...
const sendEmail = async (emailDest: string, subject: string, text: string): Promise<void> => {
  ...
  try {
    ...
    logger.info("Email envoyé", { to: emailDest, response: info.response });
  } catch (error: unknown) {
    logger.error("Échec de l'envoi de l'email", { error, to: emailDest });
    throw error;
  }
};

export { isEmailConfigured, sendEmail, verifyEmailTransport };
