...
const Dashboard = () => {
  ...
  const handleCheckboxMailChange = () => {
    ...
    const updatedUser = { ...localUser, receiveEmail: newCheckBoxState ? "true" : "false" };
    setLocalUser(updatedUser);
    updateUser({ receiveEmail: newCheckBoxState });
  }

  const highlightIcon = (elementName: string) => {
  ...
  const getBooleanFromString = (bool: string | null): boolean => {
    return bool === "true"
  }
