...
test("invalid intervals, dates and care names are rejected", () => {
  ...
  for (const date of [null, "2026-09-18", "31/02/2026", "29/02/2026", "00/12/2026", "01/13/2026", "31/12/9999"]) {
    badRequest(() => buildCareUpdate("nono", "vaccine", 1, date));
  }
  for (const care of ["notes", "__proto__", "constructor", "vermifuge", "vitamin"]) {
    badRequest(() => buildCareUpdate("nono", care, 1));
  }
});

test("legacy records accept unset intervals and models retain care fields", async () => {
  ...
  nono.set(buildCareUpdate("nono", "vaccine", 2, "18/09/2026"));
  taco.set(buildCareUpdate("taco", "antipuce", 3, "18/09/2026"));
  await nono.validate();
  await taco.validate();
  assert.equal(nono.toObject().vaccineDate, "18/09/2026");
  assert.equal(nono.toObject().vaccineIntervalMonths, 2);
