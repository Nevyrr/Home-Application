...
Le build du serveur exécute d'abord son script `prebuild`, qui réinstalle le client
depuis `server/client/package-lock.json` avec les outils nécessaires à Vite. Cela rend
le build reproductible sur Render même lorsque le service restaure un ancien cache et
exécute uniquement `npm run build` depuis `server/`.
